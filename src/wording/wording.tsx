...
export const wording = {
	index: {
		InDigitlaWorld: {
			rebirth: {
				...
				ko: (
					<>본인의 위비가 썩 맘에 들지 않으신가요? 민팅 종료 1주일 후 환생 기회가 활성화됩니다. 다시 오지 않을 두 번째 기회, 놓치지 마세요!</>
				),
			},
			weirdwine: {
				en: (
					<>A mysterious bottle of wine found in BetterWorld! Weird Wine is a utility NFT, allowing WeBe holders to expand WeBe collection in BetterWorld metaverse.</>
				),
				ko: (
					<>BetterWorld에서 신비한 와인 한 병을 발견했습니다. 메타버스에서 위비가 더 다양해질 수 있는 아이템이 제공됩니다.</>
				),
			},
			wizardwebe: {
				en: (
			...
			pioneer: {
				en: (
					<>WeBe will be the first collection to join BetterWorld, the first social media for PFPs. Communicate, post, and perform in BetterWorld via your Web 3 persona.</>
				),
				ko: (
					<>위비는 웹3 아이덴티티로 교류하는 SNS 앱 BetterWorld에 호환되는 첫 컬렉션입니다. 게시물, 좋아요/댓글, 타 PFP 팔로우/팔로잉, 개인 메타버스 공간을 통해 자유롭게 표현하고 소통할 수 있습니다.</>
				),
			},
			socializetoearn: {
				en: (
				...
				ko: (
					<>BetterWorld에서 위비는 자유롭게 표현하고 다른 유저들과 교류하며 $Klay를 보상 받을 수 있습니다.</>
				),
			},
			capsule: {
				en: (
				...
				ko: (
					<>BetterWorld에서 무료 메타버스 개인 공간인 캡슐을 부여받습니다. 본인의 위비만을 위한 공간을 꾸미고, 소통하고, 탐험하세요.</>
				),
			},
		},
		InRealWorld: {
			...
			webepartners: {
				...
				ko: (
					...
					우측 상단의 Public Docs에서 함께하기 시작한 파트너들을 확인하세요.</>
				),
			},
			weirdirl: {
				en: (
				...
				ko: (
					<>정기 오프라인 프로그램은 네트워크, 스포츠 대회, 글로벌 문화, 파티 등 다양한 주제로 구성됩니다.
					커뮤니티원은 BetterWorld 앱 내 포럼에서 다양한 프로그램 아이디어를 논의할 수 있습니다.</>
				),
			},
			weirdprojects: {
				en: (
					<>Propose, vote, and discuss weird projects in our decentralized community 
					(Off-chain forums will be available in BetterWorld).</>
				),
				ko: (
					<>포럼을 통해 아이디어를 제공하여 다른 위비들과 함께 프로젝트를 구축하세요.
					위비로서 이루고 싶은 모든 꿈은 아이디어와 파트너만 있다면 실현 가능합니다.</>
				),
			},
			webedao: {
				en: (
				...
				ko: (
					<>파트너십 후보 선정, 3D 컬렉션 디자인, 오프라인 이벤트 장소 선택을 포함한 모든 위비의 여정에 기여할 수 있습니다.
					제안 및 아이디어 선택은 해당 시기에 BetterWorld 내에서 이루어집니다.</>
				),
			},
		},
	},
	...
	story: {
		index: {
			story1: {
				ko: (
					<>지나친 경쟁과 획일화로 인해 위비는 지난 수년간 전쟁 상태에 있었습니다. 이와 같은 갈등은 이미 행성을 너무 적대적이고 삭막하여 거주하기 힘든 환경으로 만들어버렸습니다. </>
				),
				en: (
			...
			story2: {
				ko: (
					<>전쟁에 지친 8,888 마리의 위비는 이타적인 비전을 가지고 우주여행을 떠났습니다:</>
				),
				en: (
			...
			story3: {
				ko: (
					<>어느 날, 구축한 BetterWorld 캡슐에서 수면 중이던 위비는 거주 가능한 행성을 찾고 있는 다른 우주 방랑자들에 의해 깨어났습니다.</>
				),
				en: (
			...
			value: {
				ko: (
					<>8,888마리의 위비는 독창적인 문화를 전합니다. 가끔은 좀 이상해보이는 우리는 정확히 내향적이진 않지만, 대다수와 조금 다를 수 있습니다.
					요즘 우리는 좋은 대학에 가고, 안정적인 직장을 찾는 등 사회의 기대에 부응하며 숨가쁘게 살고 있죠.
					위비는 디지털과 현실 세계 모두에서 고유한 정체성을 제시하는 문화의 프랜차이즈를 만들어 이러한 사회적 규범에 도전합니다.</>
				),
				en: (
	...
	faq: {
		question: {
			...
			q5: {
				ko: (
					<>최대 몇 개의 위비를 민팅할 수 있나요?</>
				),
				en: (
					<>How many WeBe can I mint?</>
				),
			},
		},
		answer: {
			q1: {
				ko: (
					<>숫자 ‘8’은 부, 명예, ∞(무한대) 등 삶의 다양한 측면을 의미합니다. 
					위비는 다양한 의미 중에서 ∞ 로 인해 8,888을 선택했습니다. 
					변덕스러운 가격, 폰지 사기가 수두룩한 NFT 공간의 모든 외부 요인에도 불구하고 위비는 무한하고 혁신적인 커뮤니티를 보장합니다. 
					8,888 혁신가 GOMZ들과 함께 Web 3 Identity를 구축할 것입니다.</>
					),
				en: (
					<>Eight signifies various aspects of life. Wealth, prosperity, ∞ (infinity), and many more. 
					Out of all the different meanings, WeBe has chosen 8,888 due to ∞ . 
					Despite all the external factors in the NFT space, such as volatile floor prices, rug pulls, 
					and ponzi-like schemes,WeBe ensures an innovative community that will be infinite. 
					We are here to create a culture of its own with 8,888 WeBe.</>
				),
			},
			q2: {
				ko: (
					<>에너지 효율성, 짧은 지연 시간, 카카오톡 접근성, 안정적인 가스비 등의 이유로 Klaytn에서 출시하기로 결정했습니다.</>
				),
				en: (
					<>WeBe decided to launch on Klaytn for the following reasons— energy efficient, 
					low latency, easy access to Kakao Talk, and stable gas fee.</>
				),
			},
			q3: {
				ko: (
					<>위비의 민팅 과정은 4가지로 (코호트 A, 코호트 B, 코호트 C, 퍼블릭 민팅)으로 나뉘어있습니다. 코호트 A, B, C로 선정되려면 1) 코호트 회원의 초대를 받거나 2) 위비 화이트리스트 지원서를 제출하여 통과거나 3) 화이트리스트 이벤트에 선정되어야합니다.
					화이트리스트에서 퍼블릭 민팅까지, 모든 판매는 해당 공식 웹사이트에서만 이루어집니다. 2차 판매의 경우 OpenSea를 통해 제공됩니다.</>
				),
				en: (
					<>WeBe's minting process is divided into 4 categories (Cohort A, Cohort B, Cohort C, and Public Minting). 
					To be selected for Cohort A, B, or C, you must either 1) be invited by a cohort member, 2) 
					submit a WeBe whitelist application, or 3) be selected for a whitelisted event. From whitelist to public minting, 
					all sales are performed on this website. For secondary sales, it will be available via OpenSea.</>
				),
			},
			q4: {
				ko: (
					<>화이트리스트 사전 민팅: 날짜 - 공개 예정 I 가격- 100 KLAY Public 민팅: 날짜 - 8월 8일 I 가격 -  200 KLAY</>
				),
				en: (
					<>Whitelist Minting: Date- TBD I Price- 100 KLAY Public Minting: Date- August 8th I Price: 200 KLAY</>
				),
			},
			q5: {
				ko: (
					<>다양한 구성원으로 이루어진 커뮤니티를 보장하기 위해 지갑당 3개의 위비로 제한합니다.</>
				),
				en: (
					<>To ensure a wide range of community, we are limiting holders to 3 WeBe per wallet</>
				),
			},
		},
	},
