...
export const Teams = ({ team, locale, isTablet }) => {
  if (isTablet) {
    return <TeamsMobile team={team} locale={locale} />;
  }
  return <TeamsDesktop team={team} locale={locale} />;
};

const TeamsMobile = ({ team, locale }) => {
  return team(locale).map((member, index) => {
    return (
          ...
          <Div key={index} inlineBlock mx30>
            <Div flex mt20 maxW500>
              <Col itemsCenter justifyCenter>
                <Div maxH180 imgTag src={member.imageUri} roundedXl shadowLg />
              </Col>
              <Col ml30>
