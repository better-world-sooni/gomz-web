...
const PfpImage = ({imageKey, images, imageKeyLength, standardTime, isTablet, number}) => {
    ...
    const time = useTransform(standardTime, [0,100],[0+(duration+delay)*(imageKeyLength-imageKey),100+(duration+delay)*(imageKeyLength-imageKey)],{clamp:false})
    const scale = useTransform(time, value=> hover ? (changeScale+0.1):(value%totalTime<duration ? 
        changeScale
        : 1))
    const basicOpacity = useTransform(time, value=>value%totalTime<duration ? 
    1-1/duration*(value%totalTime)
    : (
        value%totalTime<totalTime/2 ?
        0
        :
    1))
    ...
    return (
        <motion.div onHoverStart={()=>{setHover(true)}} onHoverEnd={()=>{setHover(false)}} style={{width:isTablet ? "19vw":100, height:isTablet ? "19vw":100, position:"relative", scale:scaleSpringMotionValue}}>
            <Div roundedLg shadowLg style={{width:"100%", height:"100%",position:"relative"}} overflowHidden>
                <motion.div style={{width:"100%", height:"100%", position:"absolute", backgroundImage: toImageUri, backgroundSize: "cover", backgroundPositionX: "center", backgroundPositionY: "center", opacity:toOpacity}}/>
                <motion.div style={{width:"100%", height:"100%", position:"absolute", backgroundImage: basicImageUri, backgroundSize: "cover", backgroundPositionX: "center", backgroundPositionY: "center", opacity:basicOpacity}}/>
            </Div>
        </motion.div>
    )
}