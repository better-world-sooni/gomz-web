...
const GotoBetterWorldButton = ({isTablet}) => {
    ...
    const rotateMotionValue = useTransform(
        time,
        value => hover ? rotate : (value%(duration*1000)<(duration*1000/2) ? rotate:0)
        )
    const rotateSpringMotionValue = useSpring(useTransform(rotateMotionValue, value=>value-45+defaultRotate),{stiffness:300, damping:20})
    return(
        <MotionButton>
        <Div
        ...
        >	
            <motion.div style={{rotate:rotateSpringMotionValue, marginTop:2, marginRight:5}}><FaLocationArrow></FaLocationArrow></motion.div>
